...
const CustomBtn = ({label='close', classes, pathLink, fn, conditionToDisableBtn = false, children}) => {
    ...
    return (
            ...
            : 
             (<button disabled={conditionToDisableBtn} onClick={fn? executePropsFn : undefined} id={styles.customBtn} className={`btn ${classes} position-relative text-dark`}>{children? children : <span>{capitalizeString(label)}</span>}</button>)
