   ...
   const memoizePkmn = useMemoizePkmn();
    const storePkmnInformation = async (pkmnToStore, type) => {
        ...
        if(type === 'memoize') {
            ...
            const valueArr = Object.entries({flavor_text_entries: flavor_text_entries[0].flavor_text, evolves_from_species:( evolves_from_species?.name || 'base')});
            setSpecificProperties(pkmnSelectedInformation, valueArr);
